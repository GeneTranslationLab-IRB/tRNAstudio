...
dir = "../Results/Modification_Coverage/Modification_Coverage_Plots"
dir_scripts = getwd()
...
if (!file.exists("../Results/Modification_Coverage/Modification_Comparison_Plots")){
  dir.create("../Results/Modification_Coverage/Modification_Comparison_Plots", recursive=TRUE)
}
...
for(aa in aas){
  ...
  for(gene in real_genes){
    ...
    ggsave(p, file=file_name, width = 35, 
           height = 25, units = "cm", path="../Results/Modification_Coverage/Modification_Comparison_Plots" )
...
for(aa in aas){
  ...
  if(nrow(final_data)>0){
    if(table(is.na(final_data)) != length(final_data)){
      setwd("../Results/Modification_Coverage/Modification_Heatmaps_Plots")
