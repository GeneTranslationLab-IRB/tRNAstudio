...
if (!file.exists("../Results/Modification_ratio_plots")){
  dir.create("../Results/Modification_ratio_plots", recursive=TRUE)
}
...
for(group in groups){
  ...
  for(sample in sample_data$ID){
    if(sample_data$Condition[sample_data$ID==sample] == group){
      base_call_new = read.delim(paste0("../Results/R_files/Base_calling/",
                                        sample,"_all_base_calling_by_pos_CORRECT_OK.txt"),
  ...
  for(aa in aas){
    ...
    plots_cov_aa = c()
    if (!file.exists("../Results/Modification_ratio_plots/By_family")){
      dir.create("../Results/Modification_ratio_plots/By_family", recursive=TRUE)
    }
    ...
    ggsave(coverage_aa, file=paste0("../Results/Modification_ratio_plots/By_family/",
