...
# Sample Data
sample_data = read.delim("../Fastq_downloaded/sample_data.txt",
                         header=T, stringsAsFactors = T)

if (!file.exists("../Results/General_Plots")){
  dir.create("../Results/General_Plots", recursive=TRUE)
}

plot_data = c()

# Count Matrix.
counts_total <- read.delim("../Results/R_files/Counts/Counts_by_gene_total_for_DESeq2.txt")
deseqdata_total <- DESeqDataSetFromMatrix(countData=counts_total, 
                                          colData=sample_data, 
                                          design=~Condition)
...
p = ggplot(data=dge_plot, aes(y=Counts, x=Sample, fill=Group)) +
  ...
  theme(axis.text=element_text(size=16, color="black", angle=90, hjust=0,),
...
ggsave(p, file="Barplot_reads_total.jpeg", width = 35, 
       height = 25, units = "cm", path="../Results/General_Plots" )
...
for(condition in sample_data$Condition){
  ...
  p = ggplot(data=dge_plot_group, aes(y=Counts, x=Sample, fill=Group)) +
    ...
    theme(axis.text=element_text(size=18, color="black", angle=90),panel.background = element_rect(fill = "grey92"),
...
# Quality plot 
quality_plot <- ggplot(quality_data, aes(fill=MAPQ, y=Counts*100, x=Analysis)) +
...
ggsave(quality_plot, file=paste0("../Results/General_Plots/Quality_barplot.jpeg"), 
...
for(group in sample_data$Condition){
  ...
  ggsave(quality_plot, file=paste0("../Results/General_Plots/Quality_barplot_",group, 
